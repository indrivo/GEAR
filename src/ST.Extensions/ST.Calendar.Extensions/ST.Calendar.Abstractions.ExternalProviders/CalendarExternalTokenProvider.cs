...
namespace ST.Calendar.Abstractions.ExternalProviders
{
    public class CalendarExternalTokenProvider : ICalendarExternalTokenProvider
    {
        ...
        /// <returns></returns>
        public async Task<ResultModel<TTokenFormat>> GetTokenAsync<TTokenFormat>(string provider, Guid? user) where TTokenFormat : class
        {
        ...
        /// <returns></returns>
        public async Task<ResultModel> SetTokenAsync(ExternalProviderToken token)
        {
