...
namespace GR.Identity.Permissions.Abstractions.Extensions
{
    public static class UserExtensions
    {
        ...
        public static async Task<IList<Claim>> GetClaimsFromPermissions<TContext>(this GearUser user,
            TContext context) where TContext : IPermissionsContext
        {
        ...
        public static async Task RefreshClaims<TContext>(this GearUser user, TContext context,
            SignInManager<GearUser> signInManager) where TContext : IPermissionsContext
        {
        ...
        public static async Task RefreshOnlineUsersClaims<TContext>(this ClaimsPrincipal user, TContext context,
            SignInManager<GearUser> signInManager, IEnumerable<Guid> onlineUsers) where TContext : IPermissionsContext
        {
            foreach (var onlineUser in onlineUsers)
            {
                var usr = context.Users.FirstOrDefault(x => x.Id.Equals(onlineUser));
                if (usr != null)
                {
